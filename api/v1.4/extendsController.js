...
var PostsAlbumslikes = require('../../models/postsalbumslikes').postsalbumslikes;
...
exports.set_posts_albums=function(req,res){
  ...
  Sysconfigs.findOne({type:'blacklist',key:'blacklist'},function(err,sysconfigs){
    ...
    proxy.all('seq','pics',function(seq,pics){
      ...
      postsalbums.save(function(err,model){
        if(err){
          ...
          return res.send({ "code":1,message:{num:model.no,message:'ok'}});
        }
      })
    });
...
exports.set_posts_albumns_assist=function(req,res){
  ...
  console.log("forwad:"+req.headers['x-forwarded-for'] );
  var mpno=validator.trim(req.body.mpno);//用户id
  var _id=validator.trim(req.body._id);//活动相册id
  console.log("投给："+_id);
  if (!mpno || !_id)return res.send({ "code": 0,message:'Parameter is not empty!' });
  //if(useragent==='Java/1.6.0_43')return res.send({code:4,message:"您的IP投票过多"});

  var deviceAgent = req.headers["user-agent"].toLowerCase();
  if(deviceAgent.length<15){
    console.log("user-agent 太短了");
    return res.send({code:4,message:"您的IP投票过多"});
  }
  var agentID = deviceAgent.match(/(iphone|ipod|ipad|android)/);
  if(!agentID){
    console.log("不是手机投票");
    return res.send({code:4,message:"您的IP投票过多啊"});
  }
  //一个ip只允许给一人投一次
  var ip=req.headers['x-forwarded-for'],key=req.headers['x-forwarded-for']+"-"+_id+"###"+mpno+deviceAgent;
  console.log(key);
  cache.get(key,function(err,m){
    console.log(mpno);
    if(m){
      var now=Number(moment().format("X")),from=Number(m.time);
      console.log(now);
      console.log(from)
      var d=now-from;
      //小于24小时 不允许投票
      if(d<86400){
        console.log("您的IP投票过多");
        return res.send({code:4,message:'您的IP投票过多!'});
      }
    }
    var date=moment().format('YYYY-MM-DD');
    PostsAlbumslikes.count({plid:_id,mpno:mpno,time:{'$gte': date}},function(err,count){
      console.log("like："+err);
      if(err)return res.send({code:0,message:'error!'});
      if(count)return res.send({code:2,message:'已经点过赞了!'});
      Postsalbums.count({_id:_id,'user_like_this_post':{'$elemMatch':{'mpno':mpno,'time':{'$gte': date}}}},
        function(err,count){
       console.log("alb:"+err);
        if(count)return res.send({code:2,message:'已经点过赞了!'});
        var likes=new PostsAlbumslikes({
          plid:_id,mpno:mpno,time:moment(),ip:ip
        })
        likes.save(function(err){
          if(err){
            ...
            Postsalbums.update({_id:_id},{
              "$inc":{assistcount:1}
            },{ upsert: true } ,function(err){
              if(err){
                return res.send({ "code": 0,message:"save error!" });
              }else{
                cache.set(key, {ip:ip,time:moment().format('X')}, config.redis.time*60*23);//设置缓存
                return res.send({"code":1,message:"success!"});
              }
            })
          }
...
exports.get_posts_albums=function(req,res){
  ...
  if(sort==='2')query.sort('-createtime');
  if(name!==''){
  ...
  query.select({status:0,user_like_this_post:0}).skip(options.skip).limit(options.limit).exec(proxy.done(function(models){
    proxy.after( "count",models.length,function(count){
      models.forEach(function(node,index){
        node = node.toObject();
        node.assisted = 0;//未点赞
        if(count[index])node.assisted = 1;
    ...
    var date=moment().add(-8,"hours").format("YYYY-MM-DD HH:mm:ss");
    console.log(date);
    models.forEach(function(node,index){
      node = node.toObject();
      PostsAlbumslikes.count({
        plid:node._id,mpno:mpno,time:{'$gte': date}
      }).exec(proxy.group("count",function(count){
        return count;
      }));
    })
...
// 处理置顶过期的记录
function setAly(){
  var alyInterval=setInterval(function () {
      Alysjobs.findOne({status:0},function(err,model){
        ...
        if (!model) {
          console.log('==no data==')
          return clearInterval(alyInterval)
        }
...
exports.get_cooper=function(req,res){
  var options = comm.api_get_page_options(req);
  Cooper.find({status:0}).skip(options.skip).limit(options.limit).exec(function(err,models){
    if(err){
      console.log(err);
      return res.send({ "code": 0,message:"error" });
    }else{
      models.forEach(function (node, index) {
        node = node.toObject();
        node.img = comm.replace_pics(node.img);
        node.url=comm.cooper_share(node)
        models[index] = node;
      });
      return response.api(res,{}, models);
    }
