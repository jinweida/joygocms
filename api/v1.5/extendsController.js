...
exports.set_posts_albums=function(req,res){
  ...
  var contact =validator.trim(req.body.contact);//用户id
  ...
  Sysconfigs.findOne({type:'blacklist',key:'blacklist'},function(err,sysconfigs){
    ...
    Counters.findOneAndUpdate({_id:mid},{$inc:{seq:1}},{new:true,upsert: true },proxy.done(function(model){
...
exports.get_posts_albums=function(req,res){
  ...
  // }
  if(name!==''){
    if(!_.isNaN(Number(name))){  
      query.where({$or:[{'name':eval("/"+name+"/")},{'no':name}]})
    }else{
      query.where({'name':eval("/"+name+"/")});
    }
  }
...
exports.get_posts_normal_albums=function(req,res){
  var mid=validator.trim(req.query.mid);//活动id
  var type=validator.trim(req.query.type);//视频&图片 1 视频 2 图片
  var options = comm.api_get_page_options(req);
  var image=[];
  var video=[];
  var where={mid:mid,status:0}
  if(!mid)return res.send({ "code": 0,message:'mid is not empty' });
  if(!type)return res.send({ "code": 0,message:'type is not empty' });
  if (type==2) {
    where.from=10
  }else {
    where.from={$ne:10}
  }
  var query=Postsalbums.find(where);
...
exports.set_new_img=function(req,res){
  var dir='/upload/'+moment().format('YYYY-MM-DD');    
  var img=req.body.base64?comm.base64_decode({
        base64str:req.body.base64,
        absolutelydir:'./public'+dir,
        relativedir:dir,
      }):''; 
  return res.send({ "code": 1,message:img });    
}
exports.set_new_video=function(req,res){
  console.log(333)
    ...
    form.on('field', function (field, value) {
      ...
      file.path = path.join('/upload', dir, path.basename(file.path));
      file.path = file.path.replace(/\\/ig, '/');
      url=file.path.split('upload/')[1]
      url=url.split(/\.mp4/i)[0]
      url='/outfiles/'+url+'_352_500k.m3u8'
    }).on('end', function() {
      res.writeHead(200, {
