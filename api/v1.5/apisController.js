...
var _const={
    ...
    'get_medias':config.session_secret+'get_ad_'+config.version,
...
exports.get_medias = function (req, res) {
    ...
    if(cid=='80000'){
        var iosjson={
            "cmspath": "http://hele.cms.joygo.com/v1.4/api/",
            "userpath": "http://user.joygo.com/hele/",
            "appspath": "http://fuzhou.chat.joygo.com/",
            "oispath": "http://yanshi.micro.joygo.com:5000",
            "ois_android_path": "http://yanshi.micro.joygo.com:5001",
            "appname": "合乐",
            "appdep": "杰格（北京）科技有限公司",
            "appicon": "http://yichun.cdn.joygo.com/upload/neighbor/hele/icon.png",
            "applogo": "http://yichun.cdn.joygo.com/upload/neighbor/hele/logo.png",
            "appcolor": "#eb413d",
            "appdistance": "1485",
            "appimg": "http://yichun.cdn.joygo.com/upload/neighbor/hele/ad.jpg",
            "appdesc": "登上杰格云\n天涯若比邻",
            "appcoin": "宝贝",
            "appcoinunit": "个"
        }
        var androidjson={
              "cmspath": "hele.cms.joygo.com",
              "userpath": "user.joygo.com/hele",
              "appspath": "fuzhou.chat.joygo.com",
              "oispath": "yanshi.micro.joygo.com:5001",
              "appname": "合乐",
              "appdep": "杰格（北京）科技有限公司",
              "appicon": "http://yingtan.cdn.joygo.com/upload/neighbor/hele/icon.png",
              "applogo": "http://yingtan.cdn.joygo.com/upload/neighbor/hele/logo.png",
              "appcolor": "#eb413d",
              "appdistance": "1485",
              "verion": "v1.4",
              "appimg": "http://yingtan.cdn.joygo.com/upload/neighbor/hele/ad.jpg",
              "appdesc": "登上杰格云\n天涯若比邻",
              "appcoin": "宝贝",
              "appcoinunit": "个"
          }
...
exports.get_associated=function(req,res){
  var mid=validator.trim(req.query.mid);
  if (!mid) {
    return res.send({ "code": 0,message:'mid empty'});
  }
  Associated.find({pid:mid}).select({nid: 1, title: 1,postcreattime: 1,type:1}).exec(function(err,models){
    models.forEach(function (node, index) {
        node = node.toObject();
        node.postcreattime = moment(node.postcreattime).format('YYYY-MM-DD');
        node.mid=node.nid;
        node.url=comm.replace_url(node);
        delete node.mid;
        delete node.nid;
...
//获取活动主播
exports.get_hostair=function(req,res){
  var mpno=validator.trim(req.query.mpno);
  Hostair.find({pid:mpno}).select({pid: 1, mid: 1, title: 1}).exec(function(err,models){
    return res.send({
      code: 1,
      list:models
    });
...
exports.get_regtemplate=function(req,res){
  var mid=validator.trim(req.query.mid);
  var proxy=new eventproxy()
  proxy.all('regtemplate','post',function(regtemplate,post){
    if (regtemplate && post) {
      regtemplate.template.forEach(function (node, index) {
        var node = node.toObject();;
        regtemplate.template[index] = node;
      })
      return res.send({
          code: 1,
          list: regtemplate.template,
  ...
  Regtemplate.findOne({pid:mid},proxy.done(function(models){
    proxy.emit('regtemplate',models)
  }));
  Posts.findOne({_id:mid}).select({onlinepayment:1,price:1,title:1}).exec(proxy.done(function(model){
    proxy.emit('post',model)
  }))
}
//用户报名
exports.set_registration=function(req,res){
    var proxy=new eventproxy();
    var mid=validator.trim(req.body.mid);
    var mpno=validator.trim(req.body.mpno);
    var cost=validator.trim(req.body.cost);
    var num=validator.trim(req.body.num);
    var arr=validator.trim(req.body.information);
    var order=comm.getOrderNo({prefix:"FZ"});
    arr=eval('('+arr+')');
    var information=[];
    if (!mid || !arr || !mpno || !num) {
        return res.send({code:0,message:'mid or information or mpno is not empty !'})
    }
    proxy.all('post',function(post){
        registration.cost=Number(post.price)*Number(num);   
        registration.save(function(err,model){
            var postsalbums=new Postsalbums()
            postsalbums.mid=mid;
            postsalbums.rid=model._id;
            postsalbums.user.mpno=mpno;
            postsalbums.status=0;//状态为正常
            postsalbums.type=5;//为报名表
            postsalbums.save()
        })
        return res.send({ "code": 1, message: "报名成功！",order:order,cost:registration.cost});
    })
    var registration=new Registration()
    registration.pid=mid;
    registration.mpno=mpno;
    registration.order=order;
    registration.num=num?num:1;
    arr.forEach( function(node, index) {
        information.push({key:node.name,value:node.value})
    });
    registration.information=information;
    Posts.findOne({_id:mid}).select({price:1}).exec(proxy.done(function(model){
      proxy.emit('post',model)
    }))
}
//支付回调
exports.payment_notify=function(req,res){
    var _POST = req.body;
    //商户订单号
    var order_trade_no = _POST['out_trade_no'];
    // var order_trade_no = req.query.id;
    //支付宝交易号
    ...
    if(trade_status == 'TRADE_FINISHED' || 'TRADE_SUCCESS'){
        Registration.findOneAndUpdate({order:order_trade_no},{paystatus:1,tenant_order:trade_no,payment:0},function(err){})
    }
}
//报名成功福利发放
exports.payment_registration=function(req,res){
    var nums=0;
    var order=req.body.order;
    var proxy=new eventproxy();
    proxy.all('post','reg',function(post,reg){
      //activitywelfare 类型 0无 1积分 2代金券
      //activitintegral 积分数
      if (post.activitynotice==1 && reg.mpno) {
        var request = require('request');
        request.get({url:config.notice+'/sms/notice?appkey='+config.appkey+'&mobileno='+reg.mpno+'&funccode=registrationsuccess'},function(err,body){
            //发送短信后做记录
            var regnotice =new Regnotice();
            regnotice.mid=reg.pid;
            regnotice.mpno=reg.mpno;
            regnotice.save(function(err){
                if (err) {
                    console.log(err)
                }
            })
      ...
      if (post.activitywelfare==1) {
        comm.http_request_post(coin_url,{
            kind:'a',
            nums:post.activitintegral,
            subject:'活动报名',
            content :'活动报名'
      ...
      return res.send({ "code": 1, message: "报名成功！",activitintegral:nums,activitywelfare:post.activitywelfare,activitvouchers:post.activitvouchers}); 
    })
    Registration.findOne({order:order}).select({pid:1,mpno:1}).exec(proxy.done(function(result){
      proxy.emit('reg',result)
      Posts.findOne({_id:result.pid}).select({activitynotice:1,activitywelfare:1,activitintegral:1,activitvouchers:1}).exec(proxy.done(function(model){
            proxy.emit('post',model)
          }))
    }))
}
//发放代金券
exports.set_vouchers=function(req,res){
  var mpno=validator.trim(req.body.mpno);
  var vouchers=validator.trim(req.body.vouchers);
  var request = require('request');
  if (!mpno || !vouchers) {
        return res.send({code:0,message:'vouchers or mpno is not empty !'})
    }
  var form={
      mpno:mpno,
      tvdiscountruleid:vouchers,
      source:'活动报名'
  }
  request.post({url:config.envelope+'/Shake/GenerateDiscount/',form:form},function(err,response, body){
      if (!err) {
        var body=JSON.parse(body);
        var discount=body.DiscountName;
        console.log(body)
        return res.send({ "code": 1, message: "领取成功！",discount:discount});
      }else {
        return res.send({ "code": 0, message: "领取失败"});
      }
  });    
}
//获取用户报名
exports.get_registration=function(req,res){
    ...
    proxy.all('count','list',function(count,list){
      return response.apidetail(res, {}, {list:list,count:count});
    })
    Registration.count(where,function(err,count){
      proxy.emit('count',count)
    })
    Registration.find(where).sort('-createtime').skip(options.skip).limit(options.limit).select({createtime:1,mpno:1}).exec(function(err,models){
      if (models) {
        models.forEach( function(node, index) {
          var node=node.toObject()
          node.nickname='等待设置'
          node.createtime = moment(node.createtime).format('YYYY-MM-DD');
          request.get({url:config.usercenter+'/Userjsonp/ImgIDWithMpno?mpno='+node.mpno},function(err,body,response){
            var response=eval(response)
            if (response) {
              node.headimg=config.usercenter+'/user/headimg?img='+response;
              proxy.emit('mpnoFase','');
            }else {
              node.headimg='';//应该设置一个默认图片
              proxy.emit('mpnoFase','')
            }
          });
          delete node.mpno;
        ...
        proxy.after('mpnoFase',models.length,function(){
...
//获取我的活动
exports.get_registrationrecord=function(req,res){
    var options = comm.api_get_page_options(req);
    var mpno=validator.trim(req.query.mpno);
    var where={status:0}
    if (!mpno) {
        return res.send({ "code": 0,message:'mpno empty'});
    }
    where['user.mpno']=mpno;
    Postsalbums.find(where).populate(['mid','rid']).sort('-createtime').select({_id:1,mid:1,rid:1}).skip(options.skip).limit(options.limit).exec(function(err,models){
      if (models) {
        models.forEach( function(node, index) {
            var node = node.toObject();
            var order = '';
            ...
            var condition=1;
            if (node.rid) {
                if (node.rid.paystatus==0) {
                    condition=0
                }
                cost=node.rid.cost;
                order=node.rid.order;
            }
            var mid=node.mid;
            node.condition=condition;
            if (mid) {
                node.path=mid._id;
                node.title=mid.title?mid.title:'';
                node.order=order;
                node.cost=cost;
                node.activitystime=mid.activitystime?mid.activitystime:'';
                node.activityetime=mid.activityetime?mid.activityetime:'';
                node.address=mid.address?mid.address:'';
                node.pics=mid.pics?mid.pics:'';
                node.activitystime=moment(node.activitystime).format("YYYY-MM-DD");
                node.activityetime=moment(node.activityetime).format("YYYY-MM-DD");
                node.type=mid.type;
                node.pics=comm.replace_pics(node.pics);
                node.mid=mid._id;
                node.url=comm.replace_url(node);
                time=moment().format("YYYY-MM-DD");
                if (condition==0 && time>node.activityetime) {
                    node.condition=2;
                }
            }
            delete node.rid;
...
//获取我的活动详情
exports.get_registrationrecord_detail=function(req,res){
  var id=validator.trim(req.query.id);
  var where={}
  if (!id) {
      return res.send({ "code": 0,message:'id empty'});
  }
  where._id=id
  Postsalbums.findOne(where).populate(['rid']).sort('-createtime').select({'user_like_this_post':0})
    .exec(function(err,models){
        if (models) {
            var models=models.toObject();
            models.createtime=moment(models.createtime).format("YYYY-MM-DD");
            if (models.rid) {
                models.rid.createtime=moment(models.rid.createtime).format("YYYY-MM-DD");
            }
        }
      return response.apidetail(res, {}, models);
    }) 
...
exports.get_ios_neighbor=function(req,res){
  var form={
    ...
    ver:config.ver,
    platform:'ios',
    configkey:['cmspath','userpath','appspath','oispath','ois_android_path','appcoinunit','appcoin','appcolor']
  }
  request.post({url:config.neighbor,form:form},function(err,response, body){
    if (!err) {
      var body=JSON.parse(body)
      if (body.list) {
        body.list.forEach( function(node, i) {
          if (node.configs) {
            node.configs.forEach( function(el, j) {
              node[el.name]=el.config_value
            });
          }
          node.ad={};
          if (node.ads) {
            node.ads.forEach( function(em, i) {
              if (em.position_code=='neighbour_launch_01') {
                node.ad=em
              }
            });
          }
      ...
      if (body.data) {
        if (body.data.ads) {
          var ad=[]
          body.data.ads.forEach( function(node, i) {
            if (node.position_code=='neighbour_head_01') {
              ad.push(node)
            }
            delete body.data.ads
            delete body.data.configs
          });
          body.data.ad=ad
        }
      }
