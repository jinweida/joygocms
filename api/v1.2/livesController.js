...
exports.get_home=function(req,res){
  ...
  proxy.all('top','yugao','living','system','other','columns',function(top,yugao,living,system,other,columns){
    var models = {
      code: 1,
      list: []
    };
...
exports.get_lives=function(req,res){
  ...
  if(_id!==''){ //相关视频 不包含预告
    ...
    if(mpno!==''){
      ...
      if(livetype==='2')query.where({starttime:{$gte:moment().utc()}});//查看列表页 条件为预告的不显示过期的
      if(columntype!==''){
        query.where({'columns.type':columntype});//栏目为自定义的时候
...
exports.get_broadcasts=function(req,res){
  ...
  var proxy = new eventproxy();
  var cachekey=config.session_secret+'_get_broadcasts_1s.3.0';
