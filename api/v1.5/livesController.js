...
exports.get_home=function(req,res){
  ...
  //系统栏目
  Livescolumns.find({type:0,attr:0}).exec(proxy.done(function(models){
...
//查询我的微直播
exports.get_my_lives=function(req,res){
  var mpno=validator.trim(req.query.mpno);
  var options = comm.api_get_page_options(req);
  if (!mpno) {
  ...
  var sort={type:1,createtime:-1};
  var query=Lives.find({'user.mpno':mpno});//查询全部
  query.skip(options.skip).limit(options.limit).sort(sort)
  .exec(function(err,models){
    if (err || !models) {
      return res.send({ "code": 0 });
...
exports.get_lives_histories=function(req,res){
  ...
  .exec(function(err,items){
    ...
    items.forEach(function(node,index){
      if(node.livesid!=null)list.push(node.livesid);
    })
    return res.send({
      code: 1,
...
exports.set_lives=function(req,res){
  ...
  proxy.all('user','live','chatroom','sysconfigs',function(user,live,chatroom,sysconfigs){
    ...
    if(live){
      ...
      live.cds=model.cds;
      live.starttime=moment().add("100","y"); //转为直播后结束时间设置为100年以后
  ...
  proxy.all('lives','sysconfigs',function(lives,sysconfigs){
    ...
    lives.save(function(err){
      if(err)return res.send({code:0,message:'save error!'});
        ...
        if(lives.status===1){
          comm.sendMessage({chatroomid:lives.chatroom.chatroomid,info:'直播开始了！',extra:'info'},function(err,result){});
        }
        return res.send({code:1,message:'success!'});
      }
    })
...
exports.get_columns=function(req,res){
  ...
  cache.get(cachekey,function(err,get_livescolumns){
    if(get_livescolumns){
      ...
      var query=Livescolumns.find({type:0,attr:0});//获取系统栏目
...
//导播回复
exports.director_message=function(req,res){
    var body=req.body;
    var livemessage=new Livemessage()
    livemessage.nickname=body.nickname;
    livemessage.content=body.content;
    livemessage.mid=body.mid;
    livemessage.title=body.title;
    livemessage.save(function(err){
      if (!err) {
